...
3. 在拿到的内容里定位题目问的那个具体细节，逐一核对每个选项。题目问「数量/区间」时必须真的看到数字，不能估。
   **特别注意**：题目问「**某数据库按某条件筛选后的命中数/篇数**」时，
   这个数字是**该库自己的统计口径**，**绝不能**用别的数据源（OpenAlex、
   Google Scholar、Scopus、期刊官网等）的数字去凑——各库收录范围、
   文章类型划分都不同，数字对不上（实测某刊 2023 年论文数：
   ScienceDirect 口径 1497，OpenAlex 口径 1810，差了 300 多篇）。
   该库抓不到就**直接输出检索指引**，不要用替代数据源猜选项。
...
7. **题干问「在某个数据库中检索到的某文献，其文献类型标识/收录类别是什么」时**：
   必须以**该数据库记录页上自己标注的类型**为准（万方记录页会写明「会议论文」
   「期刊论文」「学位论文」等），**不能**根据文章内容、发表载体、或其它网站的
   转载页来判断。同一标题在万方可能同时存在期刊版与会议版，凭外部页面猜必错。
   抓不到该记录页就**输出检索指引**（告诉用户打开该记录页看「文献类型」字段）。
8. **题干问「错误的是 / 不正确的是 / 不包括 / 不属于」时**：要**逐项判断每个选项本身是否成立**，
...
- 小恐龙公文排版助手（Word/WPS 免费插件，官网 gw.xkonglong.com，实测可抓）：
  「排版菜单」= 点「排版」按钮旁的下拉箭头，官方功能页原文列出的子项有：
  **断行重排、删除空白、删除多余空行、全半角替换、换行转回车**（原文后接「等等」，
  说明不止这几项）。该菜单官方定位是「**尤其适合对复制自 pdf 或网页文字的快速排版**」，
  因此针对网页/PDF 粘贴产生的**超链接、图形**做清理也属于该菜单的用途范围。
  官方「功能简介」另提到「中文首行缩进2字符、删除空白，断句重排」。
